...
struct png_data read_png_file(char *filename) {
  ...
  for(int y = 0; y < height; y++) {
    ...
    //row_pointers[y] = png_malloc(png, png_get_rowbytes(png,info));
  }
...
{
  int width, height;
  width      = png_get_image_width(png_data.png, png_data.info);
  height     = png_get_image_height(png_data.png, png_data.info);
  for(int y=0; y < height; y++)
